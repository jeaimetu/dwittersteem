		...
		dbo.collection("user").findOne(findQuery, function(err, result){
			if(result.postLimit <= 0){
				...
				dbo.collection("board").insertOne(myobj, function(err, res){
					...
					cb("success");
					var limitCount = +result.postLimit - 1;
					var myObj2 = {$set: {postLimit : limitCount}};
					dbo.collection("user").updateOne(findQuery, myObj2, function(err, res1){
						db.close();
					});					
				});
			}
